...
# Generating Fourier spectra and signal to noise ratios at energies scaling from 12 to 19 J in 1 J increments. Applying known calibration to microphone to transfer to pressure signal. Applying qualitative calibration to laser to match troughs.
...
# Using low pass filtering OR bin averaging w/ mic correction
	...
	Ldata = np.array([shot.time_gate(tmin = 360e-6, tmax = 420e-6)[1] for shot in L.collection])
	Mdata = np.array([shot.time_gate(tmin = 360e-6, tmax = 420e-6)[1] for shot in M.collection])
	Ldata_noise = np.array([shot.time_gate(tmin = 300e-6, tmax = 360e-6)[1] for shot in L.collection])
	Mdata_noise = np.array([shot.time_gate(tmin = 300e-6, tmax = 360e-6)[1] for shot in M.collection]) 
	numcols = len(Ldata[0])
	avgMfft = 0
	for i in Mdata:
	...
	plt.savefig(r"C:\Users\ryans\OneDrive\Desktop\Research\Figures\No Bin Avg, No Low Pass\iter_" + str(n) + ".pdf", format = "pdf")
...
# Calculating RMS for each 
		for shot_laser, shot_micro in zip(L.collection[2:], M.collection[2:]):
			Lmax_V.append(max(shot_laser.time_gate(tmin = 420e-6, tmax = 470e-6)[1]))
			L_RMS.append(np.sqrt(np.mean(shot_laser.time_gate(tmin = 370e-6, tmax = 420e-6)[1] ** 2)))
			Mmax_V.append(max(shot_micro.time_gate(tmin = 420e-6, tmax = 470e-6)[1]))
			M_RMS.append(np.sqrt(np.mean(shot_micro.time_gate(tmin = 370e-6, tmax = 420e-6)[1] ** 2)))

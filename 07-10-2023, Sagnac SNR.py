...
# Initialize dummy arrays for SNR results as a function of fc
	...
	for fc in fc_list:
		...
		L.apply("calibrate", cal = -1/0.001, inplace = True)
...
# Calculating RMS for each 
		for shot_laser, shot_micro in zip(L.collection[2:], M.collection[2:]):
			Lmax_V.append(max(shot_laser.time_gate(tmin = 450e-6, tmax = 490e-6)[1]))
			L_RMS.append(np.sqrt(np.mean(shot_laser.time_gate(tmin = 410e-6, tmax = 450e-6)[1] ** 2)))
			Mmax_V.append(max(shot_micro.time_gate(tmin = 450e-6, tmax = 490e-6)[1]))
			M_RMS.append(np.sqrt(np.mean(shot_micro.time_gate(tmin = 410e-6, tmax = 450e-6)[1] ** 2)))
